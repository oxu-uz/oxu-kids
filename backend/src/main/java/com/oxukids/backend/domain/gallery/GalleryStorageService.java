...
public class GalleryStorageService {
    ...
    public GalleryStorageService(StorageProperties storageProperties) {
        this.galleryRoot = Paths.get(storageProperties.getUploadDir())
                .toAbsolutePath()
                .normalize()
                .resolve("gallery");
        initStorage();
    ...
    public String storeImage(MultipartFile image) {
        ...
        Path target = galleryRoot.resolve(fileName).normalize();

        try (InputStream inputStream = image.getInputStream()) {
            Files.copy(inputStream, target, StandardCopyOption.REPLACE_EXISTING);
        } catch (IOException exception) {
            throw new IllegalStateException("Rasmni saqlashda xatolik yuz berdi", exception);
        }
    ...
    public void deleteIfExists(String imagePath) {
        ...
        Path target = galleryRoot.getParent().resolve(imagePath).normalize();
        try {
            Files.deleteIfExists(target);
        } catch (IOException exception) {
            throw new IllegalStateException("Eski rasmni o'chirishda xatolik yuz berdi", exception);
        }
    }
    ...
    private void validateImage(MultipartFile image) {
        if (image == null || image.isEmpty()) {
            throw new BadRequestException("Rasm tanlanishi majburiy");
        }
